...
(defn read-transactions [conn user budget]
  (util/test-parser {:state conn
                     :auth  {:username (:user/uuid user)}} `[({:query/transactions [:db/id]} ~{:budget-uuid (:budget/uuid budget)})]))
...
(deftest read-transactions-return-conversions-matching-all
  (testing "query/transactions should return transactions and all matching conversions."
    (let [user (test-user)
          ...
          ts [(transaction budget "1000-01-01" "SEK")       ;; One conversions fetched SEK
              (transaction budget "1000-01-02" "USD")       ;; Two conversions fetched SEK+USD (user's + transactions's)
              (transaction budget "1000-01-03" "SEK")]]     ;; One conversions fetched SEK
      (transact/transact conn ts)
      (let [{result :query/transactions} (read-transactions conn user budget)
...
(deftest read-transactions-return-conversions-matching-some
  (testing "query/transactions should return transactions and all matching conversions."
    (let [user (test-user)
          ...
          ts [(transaction budget "1000-01-01" "SEK")       ;; One conversions fetched SEK
              (transaction budget "1000-01-02" "USD")       ;; Two conversions fetched SEK+USD (user's + transactions's)
              (transaction budget "1000-01-04" "SEK")]]     ;; Zero conversions fetched SEK (no conversion for date)
      (transact/transact conn ts)
      (let [{result :query/transactions} (read-transactions conn user budget)
