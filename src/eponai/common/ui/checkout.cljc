...
(defn store-checkout-element [s items]
  (dom/div #js {:className "callout cart-checkout-item"}
    ...
    (grid/row
      {:classes [::css/grid-row-align-bottom]}
      (apply grid/column {:classes [::css/grid-column-small-2 ::css/grid-column-medium-8]}
             (map (fn [i]
                    (grid/row
                      {:classes [::css/grid-row-align-top
                                 :padded
                                 :vertical]}
                      (grid/column
                        {:classes [::css/grid-column-small-4
                                   ::css/grid-column-medium-3]}
                        (photo/square (:item/img-src i)))
                      (grid/column
                        {:classes [::css/grid-column-small-5
                                   ::css/grid-column-medium-7]}
                        (dom/div #js {:className "content-item-title-section"}

                          (dom/a #js {:href      (str "/goods/" (:db/id i))
                                      :className "name"} (:item/name i))))
                      (grid/column
                        {:classes [::css/grid-column-small-3
                                   ::css/grid-column-medium-2
                                   :text-right]}
            ...
            (dom/div nil
              ...
              (when (< 1 (count (items-by-store items)))
                (dom/div #js {:className "callout transparent"}
                  ...
                  (grid/row
                    {:classes [:callout ::css/grid-row-align-bottom]}
                    (apply grid/column nil
                           (map (fn [[s its]]
                                  (grid/row
                                    {:classes [::css/grid-row-align-middle :padded :vertical]}
                                    (grid/column {:classes [::css/grid-column-small-2]}
                                      (photo/square (:store/photo s)))
                                    (grid/column nil
                                      (dom/a #js {:href (str "/store/" (:db/id s))}
                                             (dom/p #js {:className "store-name"} (:store/name s)))
                                      (common/rating-element (:store/rating s) (:store/review-count s)))
                                    (grid/column {:classes [:text-right]}
                                      (dom/span #js {:className "price"}
