...
(defn legal-entity []
  #?(:cljs
     (let [{:stripe.legal-entity/keys [dob-year dob-month dob-day first-name last-name]} form-elements
           dob-y (utils/input-value-by-id dob-year)
           dob-m (utils/input-value-by-id dob-month)
           dob-d (utils/input-value-by-id dob-day)
      ...
      (dom/div #js {:id "sulo-account-settings"}
          ...
          (my-dom/div
            ...
            (my-dom/div
              ...
              (my-dom/div
                ...
                (my-dom/div
                  (->> (css/grid-column)
                       (css/grid-column-size {:small 4 :medium 6}))
        ...
        (my-dom/div
          ...
          (my-dom/div
            (css/grid-column)
            ;(my-dom/fieldset
            ;  (css/add-class :fieldset))
            (my-dom/div
              (css/add-class ::css/callout)
              (dom/h4 nil "Personal Details")
              (my-dom/div
                (->> (css/grid-row)
                     (css/align :center))
                (my-dom/div
                  ...
                  (my-dom/div
                    ...
                    (my-dom/div
                      ...
                      (dom/input #js {:type         "text"
                                      ...
                                      :placeholder  "Day"
                                      :defaultValue (get-in stripe-account [:stripe/legal-entity :stripe.legal-entity/dob-day])}))
                    (my-dom/div
                      (css/grid-column)
                      (dom/input #js {:type         "text"
                                      :id           (:stripe.legal-entity/dob-month form-elements)
                                      :placeholder  "Month"
                                      :defaultValue (get-in stripe-account [:stripe/legal-entity :stripe.legal-entity/dob-month])}))
                    (my-dom/div
                      (css/grid-column)
                      (dom/input #js {:type         "text"
                                      :id           (:stripe.legal-entity/dob-year form-elements)
                                      :placeholder  "Year"
                                      :defaultValue (get-in stripe-account [:stripe/legal-entity :stripe.legal-entity/dob-year])}))))))))
