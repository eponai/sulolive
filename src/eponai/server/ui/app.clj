...
(defn icon [size rel href-without-size]
...
(defn apple-touch-icon [size]
  (icon size "apple-touch-icon" "/assets/img/favicon/apple-icon-"))

(defn favicon [size]
  (icon size "icon" "/assets/img/favicon/favicon-"))
    ...
    (let [{:keys [release?]} (om/props this)]
      (dom/html
       ...
       (dom/head
         ...
         ;; Favicon
         (map apple-touch-icon
              [57 60 72 76 114 120 144 152 180])
         (map favicon [16 32 96])
