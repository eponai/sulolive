...
(defn email-verification
  ([conn entity status]
    (email-verification conn entity status nil))
  ([conn entity status time-limit]
   (let [ver (f/->db-email-verification entity status time-limit)]
     (transact conn [ver])
     ver)))
    ...
    (let [fb-user (f/fb-user-db-user user-id access-token (:db/id user))]
      (if email
        (transact conn [fb-user
                        (f/->db-email-verification user :verification.status/verified)])
        (transact conn [fb-user])))
