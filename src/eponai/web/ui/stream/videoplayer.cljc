...
(defui VideoPlayer
  ...
  (componentWillUnmount [this]
    #?(:cljs
       (let [{:keys [hlsjs on-click-fn]} (om/get-state this)]
         (.destroy hlsjs)
         (.removeEventListener js/document "click" on-click-fn))))

  (componentDidMount [this]
    #?(:cljs
       (let [{:keys [on-click-fn]} (om/get-state this)
             video-element (utils/element-by-id video-element-id)]
         (when (.hls-supported? this)
           (.subscribe-hls this))

         (.setup js/plyr video-element (clj->js {:html (controls)
                                                 ;:controls ["play-large"
                                                 ;           "play"
                                                 ;           ;"progress"
                                                 ;           ;"current-time"
  ...
  (initLocalState [this]
    {:on-click-fn #(on-click-event this %)
