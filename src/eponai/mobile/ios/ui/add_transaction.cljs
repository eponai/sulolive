(ns eponai.mobile.ios.ui.add-transaction
  (:require [clojure.string :as s]
            ...
            [eponai.mobile.components :as c :refer [view modal text text-input touchable-highlight picker date-picker-ios scroll-view activity-indicator-ios picker-ios]]
            [eponai.mobile.components.button :as button]
            [eponai.mobile.components.text-field :as text-field]
    ...
    (let [{:keys [selected-project selected-input-field anim select-project? select-currency? input-transaction]} (om/get-state this)
      ...
      (view
        ...
        (view nil
              (modal
                ...
                (nav/navigator {:initial-route {:title             "lol"
                                                ...
                                                :onLeftButtonPress #(do
                                                                     (debug "Pressed select project cancel.")
        ...
        (view (opts {:style {:flexDirection "row" :marginTop 10}})
              (text-field/text-field {:onChangeText   #(do
