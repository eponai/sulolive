...
.onWrite(event => {
  const locality = event.data.val();
  const storeId = event.params.storeId;
  const collectionRef = event.data.ref.parent;
  const countPath = '/v2/' + locality + '/visitor-count/' + storeId;
...
exports.recountStoreVisitors = functions.database.ref('/v2/{locality}/visitor_count/{storeId}').onWrite(event => {
